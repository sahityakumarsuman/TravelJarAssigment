...
import material.sahitya.traveljarassigment.travelLibrary.TimeLimitException;
import material.sahitya.traveljarassigment.travelLibrary.util.CameraUtil;


public abstract class RecordingBaseActivity extends AppCompatActivity implements RecodingInterface {
    ...
    protected final void onSaveInstanceState(Bundle outState) {
        ...
        outState.putLong(Key.LENGTH_LIMIT, mLengthLimit);
    ...
    protected final void onCreate(Bundle savedInstanceState) {
        ...
        if (!CameraUtil.hasCamera(this)) {
            ...
            Toast.makeText(RecordingBaseActivity.this, R.string.capture_unsupported, Toast.LENGTH_SHORT).show();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            final int primaryColor = getIntent().getIntExtra(Key.PRIMARY_COLOR, 0);
        ...
        if (null == savedInstanceState) {
            checkPermissions();
            mLengthLimit = getIntent().getLongExtra(Key.LENGTH_LIMIT, -1);
        } else {
            ...
            mLengthLimit = savedInstanceState.getLong(Key.LENGTH_LIMIT, -1);
    ...
    public final void onBackPressed() {
        ...
        if (frag != null && frag instanceof VideoPlayFragment && allowRetry()) {
            onRetry(((CameraInterface) frag).getOutputUri());
            return;
        }
        finish();
    }
    ...
    public boolean countdownImmediately() {
        return getIntent().getBooleanExtra(Key.COUNTDOWN_IMMEDIATELY, false);
    }
    ...
    public final void onRetry(@Nullable String outputUri) {
        ...
        if (!shouldAutoSubmit() || restartTimerOnRetry())
            setRecordingStart(-1);
        if (getIntent().getBooleanExtra(Key.RETRY_EXITS, false)) {
            setResult(RESULT_OK, new Intent()
                    .putExtra(Camera.STATUS_EXTRA, Camera.STATUS_RETRY));
            finish();
            return;
        }
        getFragmentManager().beginTransaction()
    ...
    public final void onShowPreview(@Nullable final String outputUri, boolean countdownIsAtZero) {
        if ((shouldAutoSubmit() && (countdownIsAtZero || !allowRetry() || !hasLengthLimit())) || outputUri == null) {
            if (outputUri == null) {
                setResult(RESULT_CANCELED, new Intent().putExtra(Camera.ERROR_EXTRA,
                        new TimeLimitException()));
                finish();
                return;
            }
            useVideo(outputUri);
            ...
            Fragment frag = VideoPlayFragment.newInstance(outputUri, allowRetry(),
                    getIntent().getIntExtra(Key.PRIMARY_COLOR, 0));
            getFragmentManager().beginTransaction()
    ...
    public final boolean allowRetry() {
        return getIntent().getBooleanExtra(Key.ALLOW_RETRY, true);
    }

    @Override
    public final boolean shouldAutoSubmit() {
        return getIntent().getBooleanExtra(Key.AUTO_SUBMIT, false);
    }
    ...
    public final void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (grantResults[0] == PackageManager.PERMISSION_DENIED) {
            ...
            Toast.makeText(RecordingBaseActivity.this, R.string.video_perm_warning, Toast.LENGTH_SHORT).show();
    ...
    public final void useVideo(String uri) {
        if (uri != null) {
            setResult(Activity.RESULT_OK, getIntent()
                    .putExtra(Camera.STATUS_EXTRA, Camera.STATUS_RECORDED)
    ...
    public boolean restartTimerOnRetry() {
        return getIntent().getBooleanExtra(Key.RESTART_TIMER_ON_RETRY, false);
    }

    @Override
    public boolean continueTimerInPlayback() {
        return getIntent().getBooleanExtra(Key.CONTINUE_TIMER_IN_PLAYBACK, false);
    }

    @Override
    public int videoBitRate() {
        return getIntent().getIntExtra(Key.VIDEO_BIT_RATE, 1024000);
    }

    @Override
    public int videoFrameRate() {
        return getIntent().getIntExtra(Key.VIDEO_FRAME_RATE, 30);
    }

    @Override
    public float videoPreferredAspect() {
        return getIntent().getFloatExtra(Key.VIDEO_PREFERRED_ASPECT, 4f / 3f);
    }

    @Override
    public int videoPreferredHeight() {
        return getIntent().getIntExtra(Key.VIDEO_PREFERRED_HEIGHT, 720);
    }
}