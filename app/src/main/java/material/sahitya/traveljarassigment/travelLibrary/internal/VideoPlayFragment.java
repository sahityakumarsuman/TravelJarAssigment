...
public class VideoPlayFragment extends Fragment implements
        CameraInterface, View.OnClickListener, EMEventBus,
    ...
    private RecodingInterface mInterface;
    ...
    public static VideoPlayFragment newInstance(String outputUri, boolean allowRetry, int primaryColor) {
        VideoPlayFragment fragment = new VideoPlayFragment();
        fragment.setRetainInstance(true);
        Bundle args = new Bundle();
        args.putString("output_uri", outputUri);
        args.putBoolean(Key.ALLOW_RETRY, allowRetry);
        args.putInt(Key.PRIMARY_COLOR, primaryColor);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.videoplayback, container, false);
    }
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        int primaryColor = CameraUtil.darkenColor(getArguments().getInt(Key.PRIMARY_COLOR));
        ...
        mRetry.setVisibility(getArguments().getBoolean(Key.ALLOW_RETRY, true) ? View.VISIBLE : View.GONE);
        mPositionSeek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onStopTrackingTouch(SeekBar seekBar) {
                if (mWasPlaying) {
                    mStreamer.start();
                    mStreamer.startProgressPoll(VideoPlayFragment.this);
                }
            }
        });
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.playbackFrame) {
            ...
            if (mStreamer != null) {
                if (mStreamer.isPlaying()) {
                    ((ImageButton) v).setImageResource(R.drawable.aa_feed_icon_sent_unopened_blue);
                    mStreamer.pause();
                    mStreamer.stopProgressPoll();
                } else {
                    if (mFinishedPlaying)
